...
hook OnPlayerDisconnect(playerid, reason) 
{
    ...
    if (f_id != -1 && TurfWars[f_id][TW_ACTIVE] && TurfWars[f_id][TW_STARTER] == playerid) 
    {
        ...
        foreach (new i : Player)
        {
            ...
            if (PI[i][p_org_id] == TurfWars[f_id][TW_DEFENDER])
            {
...
hook OnPlayerLeaveDynArea(playerid, areaid) 
{
    ...
    GameTextForPlayer(playerid, "~r~Napustili ste teritoriju!!!~n~~n~~w~Zauzimanje prekinuto.", 1000, 3);
    SendClientMessageF(playerid, TURF_CHAT_COLOR, "(teritorija) {FF0000}Napustili ste teritoriju, zauzimanje prekinuto." );
...
hook OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    ...
    if (f_id != -1 && TurfWars[f_id][TW_ACTIVE] && playerid == TurfWars[f_id][TW_STARTER])
    {
        ...
        SetPlayerPos(playerid, pozicija[playerid][POS_X], pozicija[playerid][POS_Y], pozicija[playerid][POS_Z]+0.3);
...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    {
        GetPlayerPos(playerid, pozicija[playerid][POS_X], pozicija[playerid][POS_Y], pozicija[playerid][POS_Z]);
        SetPlayerPos(playerid, pozicija[playerid][POS_X], pozicija[playerid][POS_Y], pozicija[playerid][POS_Z]+1.5);
...
public turfWars_Tick(f_id) 
{
    ...
    if (TurfWars[f_id][TW_TIME_LEFT] == -1) 
    {
        ...
        // Upit ka bazi
        new sQuery[128];
        format(sQuery, sizeof sQuery, "UPDATE turfs SET fid = %d, level = 1 WHERE id = %d", TURFS[turf][t_fid], turf);
        mysql_tquery(SQL, sQuery, "mysql_no_return_query", "i", THREAD_TURF_UPDATE);
