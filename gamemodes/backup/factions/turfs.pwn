...
Dialog:turfedit3(playerid, response, listitem, const inputtext[]) 
{
    ...
    mysql_tquery(SQL, query); // uklonjen noreturn by daddyDOT ->, THREAD_TURF_UPDATE);
...
//Resetovanje svih zona
flags:resetzone(FLAG_ADMIN_6)
CMD:resetzone(playerid, const params[])
{
    for__loop (new i = 0; i < MAX_TURFS; i++)
    {
        //Resetovanje svih zona, true = refreshanje labela na svim zonama
        ResetTurfWars(i, true);
    }
    AdminMsg(BELA, "{0099FF}[ZONE RESET]: {FFFFFF}%s je resetovao status svih zona.", ime_rp[playerid]);
    return 1;
}