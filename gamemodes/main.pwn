/*  
    Version: v5.0
    New Maintainer: z1ann (dedic.faruk#2833)
    Past maintainers: Hoxxy & Nate; Medo & daddyDOT; zile420
    Rework and Improvement by z1ann
	Date: 25.03.2023
*/
...
public OnGameModeInit()
{
    ...
    new credits[229];
    format(credits, sizeof credits, "{81C0FF}[ NORTHERN LIGHTS v5.0 ]\n\n{FFFFFF}Owner:{81C0FF} Bozic & DomT\n{FFFFFF}Version:{81C0FF} v5.0 by z1ann {FFFFFF}- {81C0FF}/update\nAll rights reserved\n\n{FFFFFF}www.nlsamp.com");
    CreateDynamicPickup(18749, 1, 1798.6270,-1777.7433,13.5453+0.8); // Glavni spawn
    CreateDynamic3DTextLabel(credits, 0xFFFFFFAA, 1798.6270,-1777.7433,13.5453+0.5, 35.0);
...
public OnPlayerDisconnect(playerid, reason) 
{
	...
	if (IsPlayerLoggedIn(playerid))  
    {
		...
		format(query, sizeof query, "UPDATE igraci SET nivo = %d, novac = %d, banka = %d, iskustvo = %d, zavezan = %d, area = %d, zatvoren = %d,  utisan = %d, poslednja_aktivnost = CURRENT_TIMESTAMP, glad = %d, org_vreme = %d, org_rank = '%s', payday = %d, aktivnaigra = '%d|%d', ip = '%s' WHERE id = %d",
            PI[playerid][p_nivo], PI[playerid][p_novac], PI[playerid][p_banka], PI[playerid][p_iskustvo], PI[playerid][p_zavezan], PI[playerid][p_area], PI[playerid][p_zatvoren], PI[playerid][p_utisan], PI[playerid][p_glad], org_vreme, PI[playerid][p_org_rank], PAYDAY_GetTimeLeft(playerid), gettime()+300, AKTIGRA_GetPts(playerid), PI[playerid][p_ip], PI[playerid][p_id]);
		mysql_tquery(SQL, query);
...
public UpdatePlayerBubble(playerid)
{
    ...
    else if (strfind(nick, "Bozic") != -1)
    {
        format(string, sizeof string, "{FF6347}[ VLASNIK ]");
    }
    else if (strfind(nick, "Ziann") != -1)
    {
        format(string, sizeof string, "{FF6347}[ DEVELOPER ]");
    }
    else if (strfind(nick, "DomT") != -1)
    {
        format(string, sizeof string, "{FF6347}[ VLASNIK ]");
    }    
...
{
    if (IsPlayerLoggedIn(playerid) && PI[playerid][p_provedeno_vreme] <= 20)
    {
        return true;
    }
    return false;
