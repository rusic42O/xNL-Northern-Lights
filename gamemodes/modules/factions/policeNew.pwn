...
// ========================================================================== //
new
    Text3D:g_PDVehLabel[MAX_VEHICLES]
;
...
new CCTV[][E_CCTV] =
{
    {"Zlatara", {607.2593, -1438.0859, 18.3966},  {607.7253, -1438.9675, 18.1916}},
    {"Bolnica", {1176.7051, -1341.7544, 19.7045}, {1177.2001, -1340.8868, 19.3395}},
    {"PD 1",    {1539.1255, -1695.4084, 22.5653}, {1538.6173, -1694.5470, 22.0703}},
    {"PD 2",    {1607.3577, -1605.4906, 21.7286}, {1606.5525, -1606.0845, 21.4486}},
    {"Opstina", {1462.9761, -1749.7245, 20.3431}, {1463.9069, -1749.3604, 19.9981}},
    {"Banka 1", {1473.2457, -1026.9191, 28.5602}, {1472.5944, -1026.1622, 28.4702}},
    {"Banka 2", {1424.2440, -1046.5055, 32.2443}, {1425.0001, -1045.8533, 31.9594}},
    {"Burg",    {1225.0541, -875.0601, 46.3417},  { 1224.7402, -876.0092, 46.2417}}
};
...
new 
    ...
    //zlato
    tajmer:zlatoTimer,
    tajmer:zlatoTimerMob,
    zlato_vehID,
    zlatoStage = 0,
    zlatoStageReal = 0,
    zlatoPokrenuo = INVALID_PLAYER_ID,
    koImaZlato,
    zlatoUtovarArea,
    zlatoIstovarArea,
    zlatoStanje,
...
hook OnGameModeInit() 
{
    ...
    checkpoint_PDPopravka = CreateDynamicCP(1530.6863, -1677.9153, 5.8906, 2.0, 0, 0, -1, 5.0); //1587.4979,-1664.2999,13.9135
    ...
    zlatoUtovarArea = CreateDynamicCircle(598.4639, 893.0570, 5.0);
    zlatoIstovarArea = CreateDynamicCircle(874.5402, -1345.1528, 5.0);
    ...
    // PD oprema
    pickup_PDOprema = CreateDynamicPickup(1247, 1, 1401.8970,-0.9909,1001.0008, 100000);

    // FBI oprema
    pickup_SWATOprema = CreateDynamicPickup(1247, 1, 208.7232, 146.1103, 1002.9891);

    // Video nadzor
    CreateDynamic3DTextLabel("Video nadzor\n{FFFFFF}/cctv", 0xAC12F4FF, 1373.9786,-9.7760,1001.0008, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 100000);
    // 3D text za tocenje goriva PD/SWAT
    CreateDynamic3DTextLabel("[ Benzinska pumpa ]\n{FFFFFF}Upisite {FFFF00}/tocenje {FFFFFF}da natocite gorivo", 0xFFFF00FF, 1528.957153, -1686.031494, 6.473833, 7.0); // PD 1594.9836,-1662.2030,13.4703
    ...
    CreateDynamicPickup(1247, 1, 1535.5725,-1674.3898,13.3828);
    CreateDynamic3DTextLabel("[ Arrest Point ]\n{FFFF00}/uhapsi", 0x0068B3FF, 1535.5725,-1674.3898,13.3828, 12.0);
    CreateDynamicPickup(1247, 1, 1538.1968,-1644.9298,5.8906);
    CreateDynamic3DTextLabel("[ Arrest Point ]\n{FFFF00}/uhapsi", 0x0068B3FF, 1538.1968,-1644.9298,5.8906, 12.0);
    ...
    tajmer:zlatoTimer = 0;
    zlato_vehID = INVALID_VEHICLE_ID;

    //CreateEnterExit("PD",           "PD garaza", 1568.6875,-1689.9703,6.2188,88.3271, 1389.9618,0.5666,1001.0008,180.0, FACTIONS[f_id][f_ent], 0, FACTIONS[fid][f_vw], 0, 19134, 19134);
						//CreateEnterExit("PD helipad",   "PD", 1389.9618,0.5666,1001.0008,180.0, 1564.5819,-1666.1105,28.3956,  270.0, 0, FACTIONS[fid][f_ent], 0, FACTIONS[f_id][f_vw], 19134, 19134);
    //new fid = GetFactionIDbyName("Police Department");

    CreateDynamicPickup(19134, 1, 1389.9618,0.5666,1001.0008, 100000, 0, .priority = 1);
    CreateDynamicPickup(19134, 1, 1568.6875,-1689.9703,6.2188);
    CreateDynamicPickup(19134, 1, 1564.5819,-1666.1105,28.3956);

    CreateDynamic3DTextLabel("[ PD LIFT ]\nKoristite {FF6600}'Y' {FF9933}da mu pristupite", 0xFF9933FF, 1389.9618,0.5666,1001.0008, 12.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 100000, 0);
    CreateDynamic3DTextLabel("[ PD LIFT ]\nKoristite {FF6600}'Y' {FF9933}da mu pristupite", 0xFF9933FF, 1568.6875,-1689.9703,6.2188, 12.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
    CreateDynamic3DTextLabel("[ PD LIFT ]\nKoristite {FF6600}'Y' {FF9933}da mu pristupite", 0xFF9933FF, 1564.5819,-1666.1105,28.3956, 12.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
...
{
    if(koImaZlato == playerid && zlatoStanje == 1)
    {
        zlato_vehID = vehicleid;
        zlatoStanje = 0;
    }
    else if(koImaZlato == playerid && zlatoStanje == 2)
    {
        zlato_vehID = vehicleid;
        zlatoStanje = 0;
    }
...
{
    if(GetFactionAreaID(GetPlayerFactionID(playerid)) == areaid && koImaZlato == playerid && IsACriminal(playerid) && zlatoStage == 1)
    {
        new calc, ffid;

        ffid = PI[playerid][p_org_id];
        calc = get_rudar_zlato() * 700;

        if(calc > 300000) calc = 300000;

        FACTIONS[ffid][f_budzet] += calc;

        format(mysql_upit, sizeof mysql_upit, "UPDATE factions SET budzet = %d WHERE id = %d", FACTIONS[ffid][f_budzet], ffid);
        mysql_tquery(SQL, mysql_upit); // uklonjen noreturn by daddyDOT ->
        
        format(mysql_upit, sizeof mysql_upit, "UPDATE server_info SET value = 0 WHERE field = 'rudar_zlato'");
        mysql_tquery(SQL, mysql_upit); // uklonjen noreturn by daddyDOT ->

        reset_rudar_zlato();

        SendClientMessageF(playerid, HexToInt(FACTIONS[ffid][f_hex]), "(%s) {FFFFFF}Stavili ste {%s}%s {FFFFFF}u sef.", FACTIONS[ffid][f_tag], FACTIONS[ffid][f_rgb], formatMoneyString(calc));

        format(string_128, sizeof string_128, "%s |Z| AUTOSTAVLJANJE | %s | $%d (novi iznos: $%d)", FACTIONS[ffid][f_naziv], ime_obicno[playerid], calc, FACTIONS[ffid][f_budzet]);
        Log_Write(LOG_FACTIONS_SAFE, string_128);

        mysql_format(SQL, mysql_upit, 128, "INSERT INTO factions_logs (f_id, tekst) VALUES (%d, '%s je stavio %s u sef.')", ffid, ime_obicno[playerid], formatMoneyString(calc));
        mysql_tquery(SQL, mysql_upit);

        koImaZlato = INVALID_PLAYER_ID;
        zlatoStage = 0;
        zlatoPokrenuo = INVALID_PLAYER_ID;
        zlatoStanje = -1;
        zlatoStageReal = 0;

        KillTimer(tajmer:zlatoTimer);
        KillTimer(tajmer:zlatoTimerMob);

        foreach(new i: Player)
        {
            RemovePlayerMapIcon(i, MAPICON_POLICE_GOLD);
        }
    }

    if(areaid == zlatoUtovarArea && zlatoStageReal == 1 && zlatoPokrenuo == playerid)
    {
        if(GetPlayerVehicleID(playerid) == zlato_vehID)
        {
            DisablePlayerCheckpoint_H(playerid);
            TogglePlayerControllable_H(playerid, false);
            GameTextForPlayer(playerid, "~y~Zlato se utovara u kamion~n~~w~Molimo sacekajte...", 5000, 3);
            SetTimerEx("PDZlato_Utovar", 8000, false, "ii", playerid, cinc[playerid]);

            zlatoStageReal = 2;
        }
        else ErrorMsg(playerid, "Morate da doveze vozilo kojim ste pokrenuli prevoz zlata.");
    }
    
    if(areaid == zlatoIstovarArea && zlatoStageReal == 2 && zlatoPokrenuo == playerid)
    {
        if(GetPlayerVehicleID(playerid) == zlato_vehID)
        {
            DisablePlayerCheckpoint_H(playerid);
            TogglePlayerControllable_H(playerid, false);
            GameTextForPlayer(playerid, "~y~Zlato se istovara~n~~w~Molimo sacekajte...", 5000, 3);
            SetTimerEx("PDZlato_Istovar", 8000, false, "ii", playerid, cinc[playerid]);
...
hook OnPlayerEnterDynamicCP(playerid, checkpointid) 
{
    if ((checkpointid == checkpoint_PDPopravka && !PDPopravka_inUse) || (checkpointid == checkpoint_SWATPopravka && !SWATPopravka_inUse)) 
    {
        ...
        if (PI[playerid][p_novac] < 150)
        {
            TogglePlayerControllable_H(playerid, true);
            return ErrorMsg(playerid, "Nemate dovoljno novca za ovu popravku!");
        }
        ...
        else if (IsPlayerInRangeOfPoint(playerid, 15.0, 1429.7600,711189.8082,11.0527))
        {
            // SWAT
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if((newkeys & KEY_YES) && IsACop(playerid))
    {
        if(IsPlayerInRangeOfPoint(playerid, 4.0, 1389.9618,0.5666,1001.0008))
        {
            SPD(playerid, "police_lift", DIALOG_STYLE_LIST, "{FFFFFF}LIFT", "[ > ] Garaza\n[ > ] Helipad", "Odaberi", "Otkazi");
        }
        else if(IsPlayerInRangeOfPoint(playerid, 4.0, 1568.6875,-1689.9703,6.2188))
        {
            SPD(playerid, "police_lift", DIALOG_STYLE_LIST, "{FFFFFF}LIFT", "[ > ] Garaza\n[ > ] Helipad\n[ > ] Zgrada", "Odaberi", "Otkazi");
        }
        else if(IsPlayerInRangeOfPoint(playerid, 4.0, 1564.5819,-1666.1105,28.3956))
        {
            SPD(playerid, "police_lift", DIALOG_STYLE_LIST, "{FFFFFF}LIFT", "[ > ] Garaza\n[ > ] Helipad\n[ > ] Zgrada", "Odaberi", "Otkazi");
        }
...
Dialog:police_lift(playerid, response, listitem, const inputtext[])
{
    if(!response)
    {
        return true;
    ...
    if(listitem == 0)
    {
        SetPlayerCompensatedPos(playerid, 1568.6875,-1689.9703,6.2188, 0, 0);
    }
    else if(listitem == 1)
    {
        SetPlayerCompensatedPos(playerid, 1564.5819,-1666.1105,28.3956, 0, 0);
    }
    else if(listitem == 2)
    {
        new fid = GetFactionIDbyName("Police Department");

        SetPlayerCompensatedPos(playerid, 1389.9618,0.5666,1001.0008, FACTIONS[fid][f_vw], FACTIONS[fid][f_ent]);
    }
...
TaserShoot(playerid, hitid = INVALID_PLAYER_ID, bool:taserGun = false)
{
    ...
    else
    {
        ...
        if (IsPlayerRobbingBank(hitid))
        {
           ...
           // StopBankRobberyProcess(false, "Igrac koji je pljackao je taziran");
        }
        ...
        if (CallRemoteFunction("IsPlayerRobbingATM", "i", hitid))
        {
            CallRemoteFunction("ATMRobbery_Reset", "i", playerid);
        }
    }
    return 1;
}
...
stock ToggleLawDuty(playerid, bool:toggle)
{
    ...
    if (!toggle)
    {
        ...
        PlayerMoneyAdd(playerid, GetPVarInt(playerid, "oprema_novac"));
        DeletePVar(playerid, "oprema_novac");
        ...
        SetPlayerColor(playerid, 0x1D557500);
    }
}
...
public ZLATO_Timer()
{
    ...
    if (zlato_vehID == INVALID_VEHICLE_ID)
    {
        KillTimer(tajmer:zlatoTimer);
        return 1;
    }
    ...
    new Float:pos[3];
    GetVehiclePos(zlato_vehID, pos[0], pos[1], pos[2]);
    ...
    if(zlatoStage == 0)
    {
        foreach (new i : Player)
        {
            ...
            {
                RemovePlayerMapIcon(i, MAPICON_POLICE_GOLD);
                SetPlayerMapIcon(i, MAPICON_POLICE_GOLD, pos[0], pos[1], pos[2], 51, 0, MAPICON_GLOBAL);
            }
        }
    } else if(zlatoStage == 1) 
    {
        foreach (new i : Player)
        {
            if (IsACop(i) || IsACriminal(i))
            {
                RemovePlayerMapIcon(i, MAPICON_POLICE_GOLD);
                SetPlayerMapIcon(i, MAPICON_POLICE_GOLD, pos[0], pos[1], pos[2], 51, 0, MAPICON_GLOBAL);
            }
...
forward PDZlato_Utovar(playerid, ccinc); // Tajmer
public PDZlato_Utovar(playerid, ccinc) 
{
    if (DebugFunctions())
    {
        LogFunctionExec("PDZlato_Utovar");
    }
    ...
    if (checkcinc(playerid, ccinc)) {
        ...
        TogglePlayerControllable_H(playerid, true);
        GameTextForPlayer(playerid, "~g~Zlato je utovareno", 3000, 3);
        SetPlayerCheckpoint_H(playerid, 874.5402, -1345.1528, 13.6412, 10.0);
        ...
        foreach (new i : Player)
        {
            if (IsACriminal(i))
            {
                RemovePlayerMapIcon(i, MAPICON_POLICE_GOLD);
                SetPlayerMapIcon(i, MAPICON_POLICE_GOLD, pos[0], pos[1], pos[2], 51, 0, MAPICON_GLOBAL);
                SendClientMessageF(i, HexToInt(FACTIONS[PI[i][p_org_id]][f_hex]), "** Dousnik: Policija je upravo napustila rudnik u Las Venturasu, u kamionu se nalazi zlato.");
                SendClientMessageF(i, HexToInt(FACTIONS[PI[i][p_org_id]][f_hex]), "** Presretnite vozilo da ukradete zlato.");
            }
        }        
    }
    return 1;
...
forward PDZlato_Istovar(playerid, ccinc); // Tajmer
public PDZlato_Istovar(playerid, ccinc) 
{
    if (DebugFunctions())
    {
        LogFunctionExec("PDZlato_Utovar");
    ...
    if (checkcinc(playerid, ccinc)) {
        SetCameraBehindPlayer(playerid);
        TogglePlayerControllable_H(playerid, true);
        GameTextForPlayer(playerid, "~g~Zlato je istovareno", 3000, 3);
    }
    ...
    foreach (new i : Player)
    {
        if (IsPlayerInRangeOfPoint(i, 25.0, x, y, z) && IsACop(i) && IsPlayerOnLawDuty(i))
        {
            ...
            PlayerUpdateCopSkill(i, skill, SKILL_PZLATO, 0);
        }
...
Dialog:frisk_items(playerid, response, listitem, const inputtext[])
{
    ...
    {
        new cmdParams[16];
        format(cmdParams, sizeof cmdParams, "%i", targetid);
        callcmd::pretresi(playerid, cmdParams);
    }
    else
    {
        new sItemName[25],
...
Dialog:trazidozvole(playerid, response, listitem, const inputtext[])
{
    ...
    else
    {
        ...
        format(SendClientMessaged, sizeof SendClientMessaged, "%i", targetid);
        callcmd::pokazidozvole(playerid, SendClientMessaged);
    }
    return 1;
}
...
Dialog:proverivozilo(playerid, response, listitem, const inputtext[])
{
    ...
    if (ammoCount > 1000 || regStatus == 0)
    {
        ...
        foreach (new i : Player)
        {
            ...
            if (GetPlayerLastDrivenVehicleID(i) == vehicleid && IsPlayerInRangeOfPoint(i, 10.0, x, y, z) && GetPlayerAFKTime(i) < 300)
            {
...
CMD:carsign(playerid, const params[])
{
    if(!IsAdmin(playerid, 6))
    {
        ...
        if(!IsPlayerOnLawDuty(playerid))
            return ErrorMsg(playerid, "Morate biti na policijskoj duznosti da biste postavili carsign.");
    ...
    {
        g_PDVehLabel[GetPlayerVehicleID(playerid)] = CreateDynamic3DTextLabel(str, 0xFFFFFFFF, 0.0, 0.0, 0.0, 30.0, INVALID_PLAYER_ID, GetPlayerVehicleID(playerid));
    
        InfoMsg(playerid, "Uspjesno ste postavili oznaku na vozilo (%s).", str);
    }
    else
        return Koristite(playerid, "carsign [oznaka]");

    return 1;
}

CMD:removecarsign(playerid)
{
    if(!IsAdmin(playerid, 6))
    {
        if(!IsACop(playerid))
    ...
    if(!IsValidDynamic3DTextLabel(g_PDVehLabel[GetPlayerVehicleID(playerid)]))
        return ErrorMsg(playerid, "Ovo vozilo nema oznaku.");

    DestroyDynamic3DTextLabel(g_PDVehLabel[GetPlayerVehicleID(playerid)]);
    return 1;
}

CMD:ukradizlato(playerid, const params[])
{
    new Float:pos[3];

    if(zlatoStage != 1) return ErrorMsg(playerid, "Nije pokrenut prevoz zlata");
    if(IsPlayerInAnyVehicle(playerid)) return ErrorMsg(playerid, "Ne mozete krasti zlato dok ste u vozilu");
    if(zlato_vehID == INVALID_VEHICLE_ID) return ErrorMsg(playerid, "Zlato nije u vozilu.");
        
    if(IsACriminal(playerid))
    {
        GetVehiclePos(zlato_vehID, pos[0], pos[1], pos[2]);
        if(IsPlayerInRangeOfPoint(playerid, 5.0, pos[0], pos[1], pos[2]) && zlatoStanje == 0)
        {
            InfoMsg(playerid, "* Ukrali ste zlato iz kamiona, odvezite ga do baze.");
            DisablePlayerCheckpoint_H(koImaZlato);
            koImaZlato = playerid;
            zlatoStanje = 1;
            zlato_vehID = INVALID_VEHICLE_ID;
        }
        else return ErrorMsg(playerid, "Niste u blizini vozila");
    }
    ...
    if(IsPlayerOnLawDuty(playerid))
    {
        ...
        if(IsPlayerInRangeOfPoint(playerid, 5.0, pos[0], pos[1], pos[2]) && zlatoStanje == 0)
        {
            InfoMsg(playerid, "* Vratili ste zlato nazad, odvezite ga do zlatare.");
            SetPlayerCheckpoint_H(playerid, 874.5402, -1345.1528, 13.6412, 10.0);
            koImaZlato = playerid;
            zlatoStanje = 2;
            zlato_vehID = INVALID_VEHICLE_ID;
...
/*CMD:prevozzlata(playerid, const params[])
{
    new Float:pos[3];

    if(!IsACop(playerid)) return ErrorMsg(playerid, GRESKA_NEMADOZVOLU);
    if(PI[playerid][p_org_rank] == 0) return ErrorMsg(playerid, "Vi ste suspendovani sa duznosti!");
    if(!IsPlayerOnLawDuty(playerid)) return ErrorMsg(playerid, "Morate biti na duznosti da biste koristili ovu naredbu.");
    if(PI[playerid][p_org_rank] < 4) return ErrorMsg(playerid, "Potreban vam je rank 4 da biste mogli pokrenuti prevoz zlata");
    if(!IsLawVehicle(GetPlayerVehicleID(playerid))) return ErrorMsg(playerid, "Morate biti u policijskom vozilu da biste koristili ovu naredbu.");
    if(GetVehicleModel(GetPlayerVehicleID(playerid)) != 428) return ErrorMsg(playerid, "Niste u vozilu namijenjenom za ovaj posao.");
    if(!IsPlayerInRangeOfPoint(playerid, 60.0, 1566.9498, -1619.5950, 13.5469)) return ErrorMsg(playerid, "Morate biti u okrugu baze da pokrenete prevoz zlata.");
    if(get_rudar_zlato() < 50) return ErrorMsg(playerid, "Nema dovoljno zlata u rudniku da pokrenete prevoz.");
    if(zlatoStageReal != 0 && zlatoPokrenuo != INVALID_PLAYER_ID) return ErrorMsg(playerid, "Prevoz zlata je vec pokrenut.");

    FactionMsg(PI[playerid][p_org_id], "{FFFFFF}* %s je pokrenuo prevoz zlatna iz Rudnika u Las Venturasu", ime_rp[playerid]);
    FactionMsg(PI[playerid][p_org_id], "{FFFFFF}* Molimo sve jedinice koje su u mogucnosti da pruze podrsku pri prevozu zlata.");

    GetPlayerPos(playerid, pos[0], pos[1], pos[2]);

    zlato_vehID = GetPlayerVehicleID(playerid);
    tajmer:zlatoTimer = SetTimer("ZLATO_Timer", 1000, true);

    zlatoStageReal = 1;
    zlatoPokrenuo = playerid;
    ...
    SetPlayerCheckpoint_H(playerid, 598.4639, 893.0570, -42.6305, 5.0);

    InfoMsg(playerid, "* Odvezite vozilo na marker da pokupito zlato.");
...
CMD:lisice(playerid, const params[]) 
{
    ...
    if (!IsACop(playerid))
    {
        if (IsACriminal(playerid))
        {
            ...
            return callcmd::skinilisice(playerid, SendClientMessaged);
        }
    ...
    if (!gPlayerHandcuffed{targetid}) 
    {
        ...
        {    
            ...
            FactionMsg(GetPlayerFactionID(targetid), "{FF0000}Pljacka zlatare je propala jer je policija uhvatila %s.", ime_rp[targetid]);
            ...
            SetTimer("ResetJewerly", 1000, false);
            //jRob_Reset();
        }
        ...
        {    
            ...
            //StopBankRobberyProcess(false, "Igrac koji je pljackao je uhapsen");
        }
...
/*CMD:uhapsi(playerid, const params[])
{
    ...
    if (!IsPlayerInRangeOfPoint(playerid, 4.0, 1535.5725,-1674.3898,13.3828) && !IsPlayerInRangeOfPoint(playerid, 4.0, 1538.1968,-1644.9298,5.890) && !IsPlayerInRangeOfPoint(playerid, 4.0, 1771.1769,-1546.9315,9.9188))
        return ErrorMsg(playerid, "Morate biti ispred PD baze da biste izvrsili hapsenje.");
    ...
    if(IsPlayerAFK(targetid)) return ErrorMsg(playerid, "PD ne moze uhapsiti AFK igraca.");
    ...
    PI[targetid][p_zatvoren] += vreme * 2 * 60;
    ...
    if(Robber == targetid) {
        new
            bounty = floatround(GetBankRobBounty()/10000.0) * 10000 * 2;
        
        SendClientMessageFToAll(BELA, "Vesti | {FF6347}%s, koji je osumnjicen za nedavno pokusanu pljacku banke je uspjesno uhapsen od strane %s.", ime_rp[targetid], ime_rp[playerid]);
        ...
        Robber = INVALID_PLAYER_ID;
        FACTIONS[GetPlayerFactionID(playerid)][f_budzet] += bounty;
    }
    ...
    new bounty = 0;
    if (targetid == GetJewelryPoliceTarget() && GetJewelryPoliceTarget() != INVALID_PLAYER_ID)
    {
        bounty = floatround(500000/10000) * 10000 * 2;
...
CMD:uhapsi(playerid, const params[])
{
    ...
    if (!IsPlayerInArea(playerid, 1532.5935, -1708.9698, 1589.3744,-1611.1423) && !IsPlayerInArea(playerid, 1415.2610,670.9660, 1556.4900,728.3267))
    ...
    // Slanje poruke uhapsenom igracu
    SendClientMessageF(targetid, SVETLOCRVENA, "Uhapseni ste od pripadnika policije %s i kaznjeni sa %s. | Trajanje: %d min, kaucija: %s", ime_rp[playerid], formatMoneyString(cena), vreme*2, kaucijaStr);
    
    // Slanje poruke igracu koji je izvrsio hapsenje
    SendClientMessageF(playerid, SVETLOPLAVA, "Uhapsili ste %s | Trajanje: %d min, kazna: %s, kaucija: %s", ime_rp[targetid], vreme*2, formatMoneyString(cena), kaucijaStr);
...
/*
fbi enterijer
238.6384, 138.7955, 1003.0481

fbi oprema
208.7232, 146.1103, 1002.9891

fbi duty
220.7764, 146.6213, 1002.9891
    */

CMD:pdoprema(playerid, const params[]) 
{
    //  fbi                                                                         pd?
    if (!IsPlayerInRangeOfPoint(playerid, 3.0, 208.7232, 146.1103, 1002.9891) && !IsPlayerInRangeOfPoint(playerid, 3.0, 1401.8970,-0.9909,1001.0008) && !IsPlayerInRangeOfPoint(playerid, 3.0, 258.4168, 109.5337, 1003.2188))
...
CMD:duty(playerid, const params[]) 
{
    new 
        f_id = PI[playerid][p_org_id],
        r_id = PI[playerid][p_org_rank];
    ...
    if (!IsPlayerInRangeOfPoint(playerid, 3.0, 1401.8970,-0.9909,1001.0008) && !IsPlayerInRangeOfPoint(playerid, 3.0, 208.7232, 146.1103, 1002.9891))
...
CMD:kaucija(playerid, const params[])
{
    ...
    FactionMoneyAdd(0, PI[playerid][p_kaucija]);
    
    // Stavljanje van zatvora
    SetPlayerInterior(playerid, 0);
    SetPlayerVirtualWorld(playerid, 0);
    SetPlayerWorldBounds(playerid, 20000, -20000, 20000, -20000);
    SetPlayerCompensatedPosEx(playerid, 1802.7881, -1577.6869, 13.4119, 280.0, 0, 0, 3000);
    DisablePlayerCheckpoint(playerid);
    //SakrijTimerZatvorTD(playerid);
    RemovePlayerAttachedObject(playerid, 1);
    //PI[playerid][p_skin] = 137;
    ...
    PI[playerid][p_area] = 0;
    // Update podataka u bazi
    format(mysql_upit, sizeof mysql_upit, "UPDATE igraci SET kaucija = 0, zatvoren = 0, zatvor = 0 WHERE id = %d", PI[playerid][p_id]);
    mysql_tquery(SQL, mysql_upit);
    return 1;
}
...
CMD:tazer(playerid, const params[])
{
    ...
    {   
        RemovePlayerAttachedObject(playerid, SLOT_TASER);
        GivePlayerWeapon(playerid, WEAPON_SILENCED, 0);
        DestroyPlayerProgressBar(playerid, TaserChargeBar[playerid]);
...
CMD:r(playerid, params[]) 
{
    ...
    if (!IsChatEnabled(playerid, CHAT_FACTION))
        return ErrorMsg(playerid, "Iskljucili ste /r (/f) kanal. Koristite /chat da ga ponovo ukljucite.");
    ...
    {
        format(string, sizeof string, "[RADIO] (%s) {%s}%s %s[%i]: {FFFFFF}%s", FACTIONS[f_id][f_tag], boja, F_RANKS[f_id][r_id][fr_ime], ime_rp[playerid], playerid, params);
        // FactionMsg(f_id, "{%s}%s %s[%i]: {FFFFFF}%s", boja, F_RANKS[f_id][r_id][fr_ime], ime_rp[playerid], playerid, params);
    }
    else
    {
        format(string, sizeof string, "[RADIO] (%s) {%s}Rank %d %s[%i]: {FFFFFF}%s", FACTIONS[f_id][f_tag], boja, r_id, ime_rp[playerid], playerid, params);
    ...
    format(sLog, sizeof sLog, "[/r] %s: %s", ime_obicno[playerid], params);
    Log_Write(sFile, sLog);

    //new cmdParams[145];
    //format(cmdParams, sizeof cmdParams, "/r %s", params);
...
{
    if (!IsPlayerInRangeOfPoint(playerid, 5.0, 1373.9786,-9.7760,1001.0008) && pUsingCCTV[playerid] == -1)
...
CMD:ram(playerid, const params[])
{
    ...
    SetPlayerCompensatedPosEx(playerid, x, y, z, a, gid, ent, 5000);
