...
hook OnPlayerPause(playerid)
{
    ...
    SetTimerEx("CheckForAFK", false, 5000, "ii", playerid, f_id);

    return 1;
}

forward CheckForAFK(playerid, f_id);
public CheckForAFK(playerid, f_id)
{
    if(IsPlayerPaused_OW(playerid))
    {
        if (f_id != -1 && TurfWars[f_id][TW_ACTIVE] && playerid == TurfWars[f_id][TW_STARTER] && GetPlayerPausedTime_OW(playerid) > 5)
        {
            foreach (new i : Player)
            {
                if (PI[i][p_org_id] == f_id) // Poruka za napadace
...
public turfWars_Tick(f_id) 
{
    ...
    {
        new string[75], Iterator:iSkillReducing<MAX_PLAYERS>;
...
nl_public ResetTurfWars(f_id, bool:updateLabel) 
{
    ...
    foreach( new i : Player) {
        if(GetPlayerFactionID(playerid) == f_id)
            TurfState[i] = false;
    }
...
CMD:turfhelp(playerid, const params[])
{
    SPD(playerid, "no_return", DIALOG_STYLE_MSGBOX, "Turf Help", "\
        ...
        - Kako raste nivo zone, to vi-e para/oruzja dobijate od te zone.\n\n\
        \
