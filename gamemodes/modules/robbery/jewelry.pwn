...
{
    if (GetDistanceBetweenPoints(X, Y, Z, 594.6897,-1434.5972,9.8155) <= 6.0 && IsACriminal(playerid)) 
    {
        ...
        jRob_doors = CreateDynamicObject(1533, 591.321838, -1434.751708, 8.858132, -90.000000, 0.000000, 125.399986);
...
hook OnPlayerDeath(playerid, killerid, reason)
{
    ...
    else
    {
        ...
        {
            if (IsPlayerInArea(playerid, 578.2953, -1493.3750, 674.5701, -1385.6680))
            {
...
public JewelryRobEmptySafe()
{
    ...
    for__loop (new i = jRob_goldBarCounter; i < (jRob_goldBarCounter+10); i++) {
        DestroyDynamicObject(jRob_goldBar[i]);
        jRob_goldBar[i] = -1;
        Iter_Remove(iGoldBars, jRob_goldBarCounter);
    }
    jRob_goldBarCounter += 10;
    ...
    if (jRob_goldBarCounter == sizeof jRob_goldBarArray)
    {
...
{
    if (!IsPlayerInRangeOfPoint(playerid, 3.0, 601.3725, -1443.8057, 9.8155))
    ...
    SetPlayerCameraPos(playerid, 595.7446,-1437.1021,10.8155);
    SetPlayerCameraLookAt(playerid,601.3725, -1443.8057, 9.8155, CAMERA_MOVE);
